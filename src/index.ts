        ...
        if (validWallets) {
            if (balance >= wallets.length * amount) {
                for (let i = 0; i < wallets.length; i++) {
                    ...
                    } else console.log(`${chalk.bgRed.whiteBright('Minimal amount is:')}  ${networkData.withdrawMin} ${networkData.coin}, ${chalk.bgRed.whiteBright('current amount is:')} ${finalAmount} ${networkData.coin}`);
                }
            } else console.log(`${chalk.bgRed.whiteBright("Insufficient funds")}`)
        } else console.log(`${chalk.bgRed.whiteBright('Please, remove invalid wallets')}`)
    } else console.log(`${chalk.bgRed.whiteBright(`Invalid network, available networks: ${networks.join(', ')}`)}` );
})()
